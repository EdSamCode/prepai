import type { EjercicioPractico } from "@/lib/types";
...
export default function ProjectsSection({ ejercicios, titulo, subtitulo }: ProjectsSectionProps) {
  return (
    <SectionWrapper
      icon={
      ...
      titulo={titulo}
      subtitulo={`${ejercicios.length} ${subtitulo.toLowerCase()}`}
    >
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {ejercicios.map((ejercicio, i) => (
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <span
                className={`px-2 py-0.5 text-xs font-medium rounded-full border flex-shrink-0 ml-2 ${
                  DIFICULTAD_COLORS[ejercicio.dificultad] ||
            ...
            <p className="text-sm text-neutral-400">{ejercicio.descripcion}</p>

            {/* Topics */}
            <div className="flex flex-wrap gap-1.5">
              {ejercicio.temas.map((tema, j) => (
                <span
                  key={j}
            ...
            <div>
              ...
              <ul className="space-y-1">
                {ejercicio.queFuncionalidades.map((f, j) => (
                  <li
                    key={j}
            ...
            <div>
              ...
              <ul className="space-y-1">
                {ejercicio.criteriosExito.map((c, j) => (
                  <li
                    key={j}
                    className="text-xs text-neutral-500 flex items-start gap-1.5"
                  >
            ...
            <div className="flex items-center justify-between pt-2 border-t border-neutral-800">
              <span className="text-xs text-neutral-600">
                {ejercicio.tiempoEstimado}
              </span>
              <span className="text-xs text-indigo-400/80 text-right max-w-[60%]">
                {ejercicio.porQueImportante}
              </span>
            </div>
          </div>
        ))}
